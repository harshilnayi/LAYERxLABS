...
from netscope.analyzer import analyze_devices
from netscope.baseline import add_device, load_baseline, save_baseline
from netscope.nmap_runner import build_nmap_command, parse_nmap_xml, run_nmap_scan
from netscope.reporter import list_reports, write_reports


FIXTURE = Path(__file__).parent / "fixtures" / "sample_nmap.xml"


class NetScopeTests(unittest.TestCase):
    ...
    def test_run_nmap_scan_uses_built_command(self) -> None:
        ...
        with patch("netscope.nmap_runner.shutil.which", return_value="nmap"), patch(
            "netscope.nmap_runner.subprocess.run"
        ) as mock_run:
